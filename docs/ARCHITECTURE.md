...
ClaudeGauge 是一个 macOS 菜单栏小工具，**实时、状态感知地**显示 Claude Code（Pro/Max 订阅）的额度用量。它会自愈过期的 OAuth token，并且**绝不显示骗人的旧数据**——数据陈旧时菜单栏会主动变灰报警，而不是让旧数字冒充实时。
...
| 数据层 | `refresher/claude-gauge-refresh.sh` | LaunchAgent 每 30 秒 | 自适应节流拉取用量、自愈 token、原子写 `cache.json`、跨阈值推通知 | 是 |
...
- 这是核心承诺的兑现：**绝不让旧数据冒充新的**。宁可显示"我不确定"，也不显示一个可能已经过时的漂亮数字。
...
节流间隔 `iv` 的计算在 `refresher/claude-gauge-refresh.sh:55-56`：
...
- **强制绕过节流**：菜单栏点"立即刷新"或安装时传 `force` → `CQ_FORCE=1`，跳过节流判断直接 poll（`:7`、`:57`）。
- **变化检测**：`chg_now` = 5h 或周用量相比上轮变化 ≥ 1%（`:74`），写回 `state` 供下一轮判断（`:85`）。
- **节流未到点就退出**：`now - last_poll_ts < iv` 时 `raise SystemExit(0)`，本次不发请求（`:57`）。

跨阈值通知（`notify`，`:75-84`）：当某窗口 level 从低升高（`cur > prev and cur >= 1`），发一次 macOS 通知（`osascript display notification`）；标题区分 `⚠️ 紧急`（level 2）/ `需关注`（level 1），正文带 `约 X后重置`。`st[key] = cur` 保证**同一等级每轮只通知一次**，不会刷屏。

状态文件：`~/.cache/claude-gauge/refresh-state.json`（`STATE`，`:12`），存 `last_poll_ts` / `last_max_util` / `last_5h` / `last_7d` / `changed` / `notified_5h` / `notified_7d`。
...
在数据层每次运行的最前面（`refresher/claude-gauge-refresh.sh:47-53`）：
...
续命之后，正常的节流 + poll 流程接着走（`:54` 起）。如果 token 已经彻底过期（`expiresAt/1000 < now`），脚本直接退出不发请求（`:58`），等下一轮 LaunchAgent 唤醒时再试续命。
...
| `~/.cache/claude-gauge/refresh-state.json` | 数据层节流/通知状态 | 数据层 | 数据层 |
...
- **数据层通知 + 节流分级**：`refresher/claude-gauge-refresh.sh:26-30` 的 `lvl(u)`（已用值口径，`u>=90 → 2`、`u>=75 → 1`）。**两处口径相反**（一个用剩余、一个用已用），改阈值时务必两边都改、并注意换算（已用 = 100 − 剩余），否则菜单栏颜色和通知/节流会不同步。
...
- **自适应节流间隔**：`refresher/claude-gauge-refresh.sh:56` 的 `iv` 计算（45 / 60 / 60 / 240）。想更激进或更保守改这里。注意 240s 这个上限是**防 429** 的关键，不要随意调小。
- **渲染层兜底阈值**：`plugin/claude-gauge.15s.sh:127` 的 `150` 秒（best 多久没更新才让渲染层自己拉）。

### 8.4 加模型分项

数据层已经在抓 `seven_day_sonnet` / `seven_day_opus`（`refresher/claude-gauge-refresh.sh:66` 的循环 key 列表），渲染层也已展示（`plugin/claude-gauge.15s.sh:91`、`:102-105`）。要加新模型分项：

1. 在数据层 `:66` 的 key 元组里加上新窗口名（前提是 API 返回该 key）。
