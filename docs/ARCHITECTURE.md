...
- **提醒层是默认开的"离场提醒"**。默认随 `install.sh` 启用（也可用 `alert/install-alerts.sh` 单独开关）之后，Claude Code 自己的「一个回合完成 / 停下来等授权」hook 事件会触发它往 `attention.json` 写一个**时间戳 + 触发那刻的系统空闲时长**；只有当事件发生时你**既不在会话载体前台、又已经离开键鼠 ≥ `AWAY_SEC`（90 秒）**时，渲染层才把表盘点成彩虹叫你回来——这样只是临时切个窗口（你人还在、空闲≈0）不会误亮，真正起身离开才亮；左键点一下拉回会话载体、彩虹熄灭。它**只对 CC 推送的事件作反应，绝不读对话或代码**；空闲时长只取「距上次键鼠输入多久」这一个数字（IOKit HIDIdleTime），不读任何内容、不弹授权框；不联网、不弹系统通知、零遥测；无 `attention.json` 时（旧装/异常/已关）相关分支全程短路，菜单栏输出与今天逐字节一致（详见 §8.5）。
...
关键约定：**`live.json` 与 `cache.json` 结构完全一致**——都是 `{"ts": <epoch 秒>, "data": {...}}`。渲染层只比较两者的 `ts`，谁新用谁（`plugin/claude-gauge.15s.sh:224-225`）。这就是"plugin 读较新者"的全部含义：你用 CC 时桥接层写得更勤，菜单栏跟着 CC 实时刷新；你不用 CC 时数据层每分钟级别更新。
...
渲染层在跑任何渲染逻辑前，先判断「Claude 此刻是否在用」——**没在用就输出空 → SwiftBar 隐藏整个菜单栏项**，省得它在你根本不用 Claude 时常驻吃眼睛。机制在脚本顶部 `:24-47`：

- **常量**：`SEEN`（`:24`，`~/.cache/claude-gauge/seen.json`，「最近在用」时间戳）、`LINGER=120`（`:25`，Claude 退出后仍显示的秒数；设 0 = 退出即隐）。
- **`_claude_running()`（`:33-39`）**：桌面端在跑（`lsappinfo find bundleID=com.anthropic.claudefordesktop`）**或**有命令行会话存活（`pgrep -x claude`）→ `True`。**仅查进程/App 是否存在，绝不读对话/代码/凭证。**
- **`_active()`（`:40-45`）**：在跑 → 原子写 `seen.json` 时间戳并返回 `True`；没跑 → 看 `seen.json` 是否在 `LINGER` 秒内（余韵窗口）。
- **门控**：`if not _active(): raise SystemExit(0)`（`:47`）——输出空，SwiftBar 隐藏该项；正常路径不碰网络。
...
阈值常量在 `plugin/claude-gauge.15s.sh:58`：`WARN_TH, CRIT_TH = 25.0, 10.0`（剩余值口径）。分级函数 `_lvl(p)` 在 `:96-100`：`p<=10 → 2`、`p<=25 → 1`、否则 `0`。**配色刻意去绿**——够用时不是绿色而是近黑/近白，把颜色这个强信号留给真正需要注意的时刻。

### 4.3 菜单栏标题状态机（`title_line`，`plugin/claude-gauge.15s.sh:143-167`）
...
- **5 小时窗口无前缀**（即"当前"），**周窗口加 `W` 前缀**——`s5()` 与 `s7()` 函数，`:150-151`。
- **额外用量提示**：若 `extra_usage.is_enabled` 且 `used_credits > 0`，且当前是够用状态、加上后宽度仍 ≤ `MAXW`，标题追加 `+$`（`:159`）。
- 倒计时格式：5h 用 `_cd5`（分钟/`{h}h{mm}m`，≥10 小时封顶显 `9h+`，`:80-87`）；周用 `_cd7`（`{d}天{h}时` / `{d}天` / 小时 / 分钟，`:88-95`）。

### 4.4 宽度硬约束（刘海）

带刘海的 Mac，菜单栏标题一旦超过约 90px（≈ 11 字符）就会被刘海吞掉**整个消失**。所以：

- `MAXW = 11`（`plugin/claude-gauge.15s.sh:67`）是字符宽度上限。
- 宽度用 `_w(s)` 计算（`:101`），中文字符（`ord(c) > 0x2E80`）按 2 宽度算，其余按 1。
- `+$` 提示只在加上后仍不超宽时才追加（`:159`）。
- 倒计时封顶常量 `CD_CAP = "9h+"`（`:67`），避免长倒计时撑爆宽度。

### 4.5 陈旧检测（诚实陈旧）

- `STALE_SEC = 900`（15 分钟，`:21`）。
- `render()` 里 `age = time.time() - ts; stale = age > STALE_SEC`（`:176`）。
- 陈旧时：菜单栏标题变灰加 `~`（`title_line` 的 stale 分支，`:162`/`:165`），且下拉菜单插入警告块（`:186-193`）：**默认**显示 `⚠️ 数据已 N 分钟未更新` + `闲置/限流；用一下 Claude Code 即刷新`；**若 `refresh-state.json` 标了 `auth_dead`**（续命被服务端 `invalid_grant` 拒、钥匙串令牌失效）则改显 `⚠️ 登录已失效` + `在 Claude Code 里运行 /login 重新登录`——区分"需重新登录"与"闲置/限流"，不再把前者误导成后者（详见 §6.4）。
- 这样陈旧数据一眼可辨，菜单栏不会停在过时的数字上。

### 4.6 下拉菜单结构（`render` + `section`，`:169-208`）

SwiftBar 的坑：一行若**既无点击动作又无颜色**，会被渲染成"禁用灰"。所以**每一行都显式上色**。

每个窗口由 `section(label, icon, u, cd_str, col)` 渲染三/四行（`:169-173`）：
...
进度条函数 `bar(used)`（`:70-71`）：`f = round(used/10)`，输出 `"█"*f + "░"*(10-f)`。

下拉还包含：标题行 `Claude Code 用量`；按模型分项（若有 `seven_day_sonnet` / `seven_day_opus`，`:197-200`）；更新时间 `更新于 HH:MM（N分钟前/刚刚）`（`:202-203`）；一个可点击的 **`立即刷新（强制拉最新）`** 项——它 `shell` 调用 `~/.claude/claude-gauge-refresh.sh` 并传 `param0=force`，强制数据层立刻 poll（`:204`）；以及（仅当稳定卸载脚本 `~/.claude/claude-gauge-uninstall.sh` 存在时）一个 **`管理 ▸ 卸载 ClaudeGauge…`** 子菜单——`terminal=true` 在 Terminal 里可见地跑卸载脚本，收进子菜单不污染主下拉（`:205-208`，机制见 §8.6）。
...
节流间隔 `iv` 的计算在 `refresher/claude-gauge-refresh.sh:88-93`：
...
# 连续 429 自动退让（防恶性循环）
if poll_fail_streak >= 3: iv = max(iv, 600)   # 10 分钟
elif poll_fail_streak >= 1: iv = max(iv, 300) # 5 分钟
```
...
- **强制绕过节流**：菜单栏点"立即刷新"或安装时传 `force` → `CQ_FORCE=1`，跳过节流判断直接 poll（`:8`、`:94`；`refresh` 测试钩子的 `CQ_REFRESH=1` 同样绕过，`:9`、`:94`）。
- **429 重试与退让**：poll 遇 429 时重试一次（15s 后），仍失败则递增 `poll_fail_streak` 并写 state（`:108-122`）。连续失败时自动拉长节流间隔（1 次→300s、3 次→600s），防止越限流越密集。成功 poll 清零 `poll_fail_streak`（`:152`）。
- **变化检测**：`chg_now` = 5h 或周用量相比上轮变化 ≥ 1%（`:148-151`），写回 `state` 供下一轮判断（`:152`）。
- **节流未到点就退出**：`now - last_poll_ts < iv` 时 `raise SystemExit(0)`，本次不发请求（`:94`）。

状态文件：`~/.cache/claude-gauge/refresh-state.json`（`STATE`，`:14`），存 `last_poll_ts` / `last_max_util` / `last_5h` / `last_7d` / `changed` / `poll_fail_streak`。
...
续命之后，正常的节流 + poll 流程接着走（`:88` 起）。如果 token 已彻底过期且续命也失败（例如 refresh token 本身失效、需要用户重新登录 CC），脚本把 `auth_dead` 写进 `refresh-state.json` 供菜单栏提示 `/login`（详见 §6.4），再直接退出不发请求（`:95`），菜单栏走"诚实陈旧"变灰，等用户下次用 CC 重新登录后自动恢复。

### 6.3 渲染层的独立兜底

渲染层也能读 token 自己拉一次（`read_token`，`plugin/claude-gauge.15s.sh:214-221`），但**仅在后台缓存彻底失效时**才触发——`best is None` 或 `best.ts` 比当前早超过 150 秒（`:226`）。它读 token 前会检查 `expiresAt` 是否还有 30 秒余量（`:228`），过期就不拉。注意渲染层**不做 token 续命**，续命是数据层的专责。

### 6.4 续命彻底失败时的"诚实失败态"（auth_dead → 提示 /login）

续命并非永远能成功。最常见的彻底失败：**钥匙串里的 refresh token 已被服务端作废**——CC 活跃使用时会自己轮换 OAuth token，新 token 只在 CC 内存里、**未必回写钥匙串**；于是钥匙串可能停在一个「access token 已过期 + refresh token 已失效」的死状态。此时续命 POST `…/oauth/token` 会收到 **HTTP 400 `{"error":"invalid_grant"}`**，且**无法自愈**（有效 token 只在 CC 内存里，外部进程读不到；唯有用户在 CC 里 `/login` 重新登录才能在钥匙串里重铸一对有效 token）。

为避免把这种状态误导成"闲置/限流"，数据层与渲染层这样协作：

- **数据层标记**：`refresh_oauth()` 返回值改成 `(新 claudeAiOauth 或 None, 是否 invalid_grant)`——只有收到 `400 invalid_grant` 才置 `dead=True`（网络抖动等其它失败**不**置位，避免误报）。主流程据此把 `auth_dead` 写进 `refresh-state.json`（紧跟续命调用、早于节流/退出持久化，`:84` 一带）；任何一次成功 poll 的收尾 `st.update` 会把 `auth_dead` 清回 `False`（`:152`），故用户重新登录后自动复原。失败路径**绝不写 keychain**。
- **渲染层分流**：陈旧分支读 `refresh-state.json` 的 `auth_dead`——为真则把下拉警告从「闲置/限流；用一下 Claude Code 即刷新」换成「⚠️ 登录已失效 / 在 Claude Code 里运行 `/login` 重新登录」（`plugin/claude-gauge.15s.sh:186-193`）。这是关键体验修复：旧文案在"令牌失效"时让用户"用一下 CC 就好"，但用户正在用 CC 也好不了，反而困惑。
- **覆盖盲区（诚实标注）**：这条只把"卡死"变成"明确指引"，**不恢复数据**——钥匙串路径无法自动恢复。真正对此 bug 免疫的是桥接层（零 token，直接从 CC 取 `rate_limits`），但桥接**仅对终端 CLI 会话生效；桌面版 Claude.app 的会话不执行命令型 `statusLine`**，故桌面端用户遇到此态时只能靠本提示去 `/login`。
...
| `~/.cache/claude-gauge/attention.json` | 提醒层未读事件 `{ts, event, front, host, idle}`（`idle` = 触发那刻的系统空闲秒数；默认开；无此文件时＝旧装/异常/已关） | 提醒层（hook 触发 `event` 时） | 渲染层 |
...
- **渲染层显示分级**：`plugin/claude-gauge.15s.sh:58` 的 `WARN_TH, CRIT_TH = 25.0, 10.0`（剩余值口径，对应已用 75% / 90%）。改这里会改菜单栏的橙/红切换点。
- **数据层节流分级**：`refresher/claude-gauge-refresh.sh:90` 的 `iv` 计算，直接用 `last_max_util` 配合内联判断（已用值口径，`>=90` 紧急 45s、`>=75` 需关注 60s）。**两处口径相反**（一个用剩余、一个用已用），改阈值时务必两边都改、并注意换算（已用 = 100 − 剩余），否则菜单栏颜色和节流频率会不同步。

### 8.2 改配色

`plugin/claude-gauge.15s.sh:60`：`COL_WARN="#e08a2b"`（橙）、`COL_CRIT="#e0483d"`（红）、`COL_STALE="#9a9a9a"`（灰）。够用色在 `:65` 的 `NORMAL`（深色 `#ededef` / 浅色 `#1d1d1f`，由 `_is_dark()` 判定）。辅助灰 `MUTE` 在 `:66`。配色"去绿"是刻意设计，改时建议保持够用态为低饱和度中性色。

### 8.3 改轮询策略

- **后台唤醒频率**：`install.sh:42` 的 `<key>StartInterval</key><integer>30</integer>`。这是 launchd 唤醒脚本的间隔，不是实际 poll 间隔。
- **自适应节流间隔**：`refresher/claude-gauge-refresh.sh:90` 的 `iv` 计算（45 / 60 / 60 / 240）。想更激进或更保守改这里。注意 240s 这个上限是**防 429** 的关键，不要随意调小。连续失败时 `:91-93` 自动退让到 300s/600s。
- **渲染层兜底阈值**：`plugin/claude-gauge.15s.sh:226` 的 `150` 秒（best 多久没更新才让渲染层自己拉）。

### 8.4 加模型分项

数据层已经在抓 `seven_day_sonnet` / `seven_day_opus`（`refresher/claude-gauge-refresh.sh:132-139` 的循环），渲染层也已展示（`plugin/claude-gauge.15s.sh:178`、`:197-200`）。要加新模型分项：

1. 在数据层 `:99` 的 key 元组里加上新窗口名（前提是 API 返回该 key）。
2. 在渲染层 `render()` 里仿照 `son`/`opus` 取 `remain(...)` 并加进 `extras` 列表（`:178`、`:197-200`）。
...
- `event <stop|permission>`（被 CC 的 hook 调用，`main` `:117-125`）：**绝不读 stdin**——CC 会把含 `transcript_path` 的 JSON 灌进 stdin，本脚本整条忽略；事件类型由 hook 的 matcher 在 CC 层就分好了。原子写 `attention.json = {ts, event, front, host, idle}`（`:123`）：`front` 是 `front_bundle()`（`:72-90`，用 `lsappinfo`、不弹授权框）取的**触发那刻前台 App 的 bundle id**，取不到记 `"unknown"`（宁可多提醒、不漏）；`host` 是 `session_host()`（`:29-57`）走**进程祖先链**（`ps -o ppid=`/`comm=`）认出的**本次会话宿主 App bundle**——终端会话→终端 App（Terminal/iTerm/VSCode 等），桌面会话→`com.anthropic.claudefordesktop`（经 `/Applications/Claude.app/.../disclaimer` 解析），跳过 claude CLI 自身（路径含 `/claude-code/`）与 shell/解释器，认不出记 `None`；`idle` 是 `idle_secs()`（`:93-105`）读的**触发那刻系统空闲秒数**——经 `ioreg -c IOHIDSystem` 取 IOKit `HIDIdleTime`，**只读「距上次键鼠输入多久」这一个时长，不读任何内容、不弹授权框**，取不到记 `0.0`（视作"你在用"，宁可不打扰也不误判离开）。同时顺带原子写 `seen.json`（`:124`）喂 ① 的 linger，让刚完成时彩虹在 linger 窗口内不被 ① 隐藏。写完 `open -g swiftbar://refreshplugin?name=claude-gauge.15s.sh` 让菜单栏即时重画。
- `open`（被菜单栏左键点击调用，`:126-141`）：从 `attention.json` 读 `host`，`open -b <host>`（回退链 host→front→桌面端；`host`/`front` 缺失或 `"unknown"` 时回落桌面端 `com.anthropic.claudefordesktop`），把你拉回会话所在载体——**终端会话点击回终端、桌面会话回桌面端**（关键修复：旧版写死拉桌面端，终端会话点了会跳错 App 甚至无反应）。再原子写 `ack.json = {ts}` 确认已读 → 彩虹熄灭。

#### 缓存契约（均在 `~/.cache/claude-gauge/`，原子写）

- `attention.json = {"ts": <epoch 秒>, "event": "stop"|"permission", "front": <bundle id 或 "unknown">, "host": <会话宿主 bundle id 或 null>, "idle": <触发那刻系统空闲秒数 float>}` —— 提醒层（hook `event`）写，渲染层读。`host` 是判定点亮/熄灭/点击目标的会话载体；缺失（旧数据 / 认不出）则各处回退老行为（与 `CLAUDE_BUNDLE` 比较、拉桌面端）。`idle` 是判定"你是否真离开"的空闲门——缺失（旧 attention.json / 续装前数据）读作 `0` → 不点亮（fail-quiet）。
- `ack.json = {"ts": <epoch 秒>}` —— 提醒层（左键点击 `open`）写、渲染层（检测到会话载体回到前台时）也写；渲染层读。
- `seen.json = {"ts": <epoch 秒>}` —— ① 随 Claude 显隐用的「最近在用」时间戳；提醒层 `event` 时顺带写一份（让刚完成的彩虹在 linger 窗口内不被 ① 隐藏），渲染层 / 数据层各自也写读（见渲染层 §4 顶部 `_active`、`:40-47`）。

#### 渲染层点亮判定（`plugin/claude-gauge.15s.sh`）

无 `attention.json` 时（旧装/异常/已关），`_armed()` 恒 `False`、`render()` 里相关分支不进，零额外开销。

- **`_armed()`（`:135-141`）**：要点亮彩虹须**同时**满足三条——(a) `attention.front != (attention.host or CLAUDE_BUNDLE)`（事件发生时你不在**会话载体**前台；`host` 缺失则回退比桌面端 bundle，`:139`），**且** (b) `attention.idle >= AWAY_SEC`（事件发生时你已离开键鼠 ≥ `AWAY_SEC`，`:140`；`AWAY_SEC=90.0` 是紧挨 `def _armed()` 上方的可调常量，`:134`），**且** (c) `attention.ts > ack.ts`（未读，`:141`）→ 返回 `True`。`idle` 门是新加的：旧规则只看「完成那一刻 Claude 窗口是否前台」，于是你人在、只是临时切到别的 App（如 Chrome）也会误亮；`idle >= 90s` 把「真的起身离开」和「在用电脑、只是切了下窗口（空闲≈0）」区分开。缺/旧 `idle` 字段读作 `0` → 不点亮（fail-quiet）。
- **回到会话载体自动熄灭**：`render()`（自动熄灭判定在 `:181-182`）每次重画时若检测到当前前台 == `attention.host or CLAUDE_BUNDLE`（`_front_bundle()`，`:113-123`，同样只用 `lsappinfo`）且有未读，就写一份新的 `ack.json` → 下次重画 `_armed` 转 `False`、彩虹熄灭（≤15s 内自动恢复常态）。即终端会话回终端、桌面会话回桌面端都能自动熄灭。此自动熄灭逻辑未受 idle 门影响。
- **armed 渲染**（`title_line` 的 armed 分支，`:160-164`）：菜单栏标题用 `image={ICON_RAINBOW} {ICON_SZ}` 全彩位图 **+ 左键动作** `bash=/usr/bin/python3 param0=<alert.py> param1=open terminal=false`；**数字仍保留额度三色**（橙/红恒显，够用态不写 `color=` 走菜单栏自适应），只把图标染彩虹——两个信号共存，不遮蔽额度告急。普通（未 armed）态走原版图标常量（够用态 `templateImage={ICON_OK}`、橙/红/灰 `image={ICON_WARN/CRIT/STALE}`，`:165-166`），与 `ICON_RAINBOW` 同源同形（均由 `build-menubar-icons.sh` 从 `docs/logo.svg` 渲），故彩虹态与普通态**形状 100% 一致**。
- **彩虹位图来源**：`ICON_RAINBOW`（`:57`，内嵌 base64）由 `alert/build-menubar-icons.sh` 构建期一次性生成——从品牌 logo `docs/logo.svg`（分段光谱仪表盘，与落地页/favicon 同形）用 `rsvg-convert` 渲出彩虹弧 + 深针 + 白芯，描边按菜单栏重量减细（弧 1.8 / 针 1.44，与 SF 符号同栏和谐）。它和 OK/WARN/CRIT/STALE 四态由**同一脚本同一形状**渲出，只是着色不同，故彩虹态与普通态形状 100% 一致。**为何用 `image=` 全彩位图而非 SF 符号 sfconfig**：SF 符号的 Palette 多色在实测 SwiftBar 上糊成单橙，故彩虹/橙/红/灰态都用 `image=` 全彩位图，够用态用 `templateImage` 单色蒙版随栏自适应（见 `:48-51` 注释）。**运行期零依赖**——插件只读内嵌 base64，`build-menubar-icons.sh` 仅在重生成图标时本机用一次（仅构建期需 `librsvg`）。

#### 触发源 / 装卸

- **触发源**：`Stop`（一个回合完成）+ `Notification`/`permission_prompt`（终端模式卡住等授权）+ `PermissionRequest`（桌面端授权弹窗的真实触发点），**不订阅 CC 的 `idle` hook 事件**（与"完成"重复；注意这与上面写进 `attention.json` 的系统空闲门 `idle` 字段无关，后者只是触发那刻的空闲时长）。只关注「有/无」未读，不区分是哪个 / 几个会话。
...
- **稳定副本**：`install.sh:29` 用 `install -m 0755 "$REPO/uninstall.sh" "$HOME/.claude/claude-gauge-uninstall.sh"`——即使 clone 目录被删/移动，卸载脚本仍在。`uninstall.sh` 结尾 `rm -f "$HOME/.claude/claude-gauge-uninstall.sh"`（`uninstall.sh:94`）自删这份副本，卸得干净。
- **管理子菜单**：渲染层 `render()`（`:205-208`）在「立即刷新」之后，**仅当** `~/.claude/claude-gauge-uninstall.sh` 存在时，打印一个 `管理 ▸ 卸载 ClaudeGauge…` 子菜单（`shell=/bin/bash param0=<那份稳定副本> terminal=true`）。`terminal=true` 让卸载在 Terminal 里可见地跑（用户能看到输出与提示）；用子菜单收纳、不污染主下拉。
- **对称移除提醒层 hook**：主卸载脚本以一段**内联、自包含**（不依赖 repo 仍存在）的 python 块对称移除我们加的 hook——只剥掉 command 含 `claude-gauge-alert.py` 的 `Stop`/`Notification`/`PermissionRequest` 条目（先备份、回解析校验、原子写、空组/空事件不留空壳），**你已有的任何其它 hook 原封不动**（`uninstall.sh:9-45`，非致命：settings.json 异常只提示、不阻断卸载），随后 `rm -f "$HOME/.claude/claude-gauge-alert.py"`（`uninstall.sh:46`）。**statusLine 桥接也对称自动移除**：另一段同样自包含的 python 块（`uninstall.sh:48-65`）**仅当** `settings.json` 的 `statusLine` 指向本桥接（command 含 `claude-gauge-statusline.py`）时才删该键（先备份、回解析校验、原子写），你自定义的 `statusLine` 原封不动。
...
- 默认开的**提醒层**（§8.5）只对 CC 自己推送的 `Stop`/`Notification`/`PermissionRequest` hook 事件作反应、只记一个时间戳 + 事件名 + 前台/会话宿主 App 的 bundle id + 触发那刻的系统空闲秒数（**面向会话载体 ⑤** 只读进程元数据 `lsappinfo`/`ps`/`defaults`；空闲秒数经 `ioreg` 读 IOKit `HIDIdleTime`，**只是「距上次键鼠输入多久」一个时长**，都不弹辅助功能/自动化授权框），**绝不读 stdin/transcript/对话/代码**；纯本机 hooks，不联网、不弹系统通知、零遥测。
