...
- **稳定副本**：`install.sh:29` 用 `install -m 0755 "$REPO/uninstall.sh" "$HOME/.claude/claude-gauge-uninstall.sh"`——即使 clone 目录被删/移动，卸载脚本仍在。`uninstall.sh` 结尾 `rm -f "$HOME/.claude/claude-gauge-uninstall.sh"`（`uninstall.sh:52`）自删这份副本，卸得干净。
...
- 卸载（`uninstall.sh`）只删自己装的东西（含对称移除 command 含 `claude-gauge-alert.py` 的提醒层 hook，先备份、回解析校验、原子写，**你已有的其它 hook 原封不动**，`uninstall.sh:9-45`），**不碰** Claude Code 的凭证与数据；statusLine 是用户手动加的那一行，需用户自行从 `~/.claude/settings.json` 移除（`uninstall.sh:51`）。
...
**安装**：`git clone` 后 `./install.sh`。脚本会：检测/安装 SwiftBar → 解析 SwiftBar 插件目录 → 装三个组件 → 写并加载 LaunchAgent → `force` 拉一次首数据 → 默认启用提醒层（step 6 复用 `alert/install-alerts.sh` 合并 hook，非致命） → 提示可选的 statusLine 配置。
