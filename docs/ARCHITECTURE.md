...
节流间隔 `iv` 的计算在 `refresher/claude-gauge-refresh.sh:88-89`：
...
- **强制绕过节流**：菜单栏点"立即刷新"或安装时传 `force` → `CQ_FORCE=1`，跳过节流判断直接 poll（`:8`、`:90`；`refresh` 测试钩子的 `CQ_REFRESH=1` 同样绕过，`:9`、`:90`）。
- **变化检测**：`chg_now` = 5h 或周用量相比上轮变化 ≥ 1%（`:107`），写回 `state` 供下一轮判断（`:108`）。
- **节流未到点就退出**：`now - last_poll_ts < iv` 时 `raise SystemExit(0)`，本次不发请求（`:90`）。
...
续命之后，正常的节流 + poll 流程接着走（`:87` 起）。如果 token 已彻底过期且续命也失败（例如 refresh token 本身失效、需要用户重新登录 CC），脚本把 `auth_dead` 写进 `refresh-state.json` 供菜单栏提示 `/login`（详见 §6.4），再直接退出不发请求（`:91`），菜单栏走"诚实陈旧"变灰，等用户下次用 CC 重新登录后自动恢复。
...
- **数据层标记**：`refresh_oauth()` 返回值改成 `(新 claudeAiOauth 或 None, 是否 invalid_grant)`——只有收到 `400 invalid_grant` 才置 `dead=True`（网络抖动等其它失败**不**置位，避免误报）。主流程据此把 `auth_dead` 写进 `refresh-state.json`（紧跟续命调用、早于节流/退出持久化，`:84` 一带）；任何一次成功 poll 的收尾 `st.update` 会把 `auth_dead` 清回 `False`（`:108`），故用户重新登录后自动复原。失败路径**绝不写 keychain**。
...
- **数据层节流分级**：`refresher/claude-gauge-refresh.sh:89` 的 `iv` 计算，直接用 `last_max_util` 配合内联判断（已用值口径，`>=90` 紧急 45s、`>=75` 需关注 60s）。**两处口径相反**（一个用剩余、一个用已用），改阈值时务必两边都改、并注意换算（已用 = 100 − 剩余），否则菜单栏颜色和节流频率会不同步。
...
- **自适应节流间隔**：`refresher/claude-gauge-refresh.sh:89` 的 `iv` 计算（45 / 60 / 60 / 240）。想更激进或更保守改这里。注意 240s 这个上限是**防 429** 的关键，不要随意调小。
- **渲染层兜底阈值**：`plugin/claude-gauge.15s.sh:226` 的 `150` 秒（best 多久没更新才让渲染层自己拉）。

### 8.4 加模型分项

数据层已经在抓 `seven_day_sonnet` / `seven_day_opus`（`refresher/claude-gauge-refresh.sh:99` 的循环 key 列表），渲染层也已展示（`plugin/claude-gauge.15s.sh:178`、`:197-200`）。要加新模型分项：
