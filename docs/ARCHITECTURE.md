...
节流间隔 `iv` 的计算在 `refresher/claude-gauge-refresh.sh:34-35`：
...
- **强制绕过节流**：菜单栏点"立即刷新"或安装时传 `force` → `CQ_FORCE=1`，跳过节流判断直接 poll（`:6`、`:36`）。
- **变化检测**：`chg_now` = 5h 或周用量相比上轮变化 ≥ 1%（`:53`），写回 `state` 供下一轮判断（`:54-55`）。
- **节流未到点就退出**：`now - last_poll_ts < iv` 时 `raise SystemExit(0)`，本次不发请求（`:36`）。

状态文件：`~/.cache/claude-gauge/refresh-state.json`（`STATE`，`:11`），存 `last_poll_ts` / `last_max_util` / `last_5h` / `last_7d` / `changed`。
...
- **数据层节流分级**：`refresher/claude-gauge-refresh.sh:35` 的 `iv` 计算，直接用 `last_max_util` 配合内联判断（已用值口径，`>=90` 紧急 45s、`>=75` 需关注 60s）。**两处口径相反**（一个用剩余、一个用已用），改阈值时务必两边都改、并注意换算（已用 = 100 − 剩余），否则菜单栏颜色和节流频率会不同步。
...
- **自适应节流间隔**：`refresher/claude-gauge-refresh.sh:35` 的 `iv` 计算（45 / 60 / 60 / 240）。想更激进或更保守改这里。注意 240s 这个上限是**防 429** 的关键，不要随意调小。
- **渲染层兜底阈值**：`plugin/claude-gauge.15s.sh:127` 的 `150` 秒（best 多久没更新才让渲染层自己拉）。

### 8.4 加模型分项

数据层已经在抓 `seven_day_sonnet` / `seven_day_opus`（`refresher/claude-gauge-refresh.sh:45` 的循环 key 列表），渲染层也已展示（`plugin/claude-gauge.15s.sh:91`、`:102-105`）。要加新模型分项：

1. 在数据层 `:45` 的 key 元组里加上新窗口名（前提是 API 返回该 key）。
