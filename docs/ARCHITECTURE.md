...
关键约定：**`live.json` 与 `cache.json` 结构完全一致**——都是 `{"ts": <epoch 秒>, "data": {...}}`。渲染层只比较两者的 `ts`，谁新用谁（`plugin/claude-gauge.15s.sh:217-218`）。这就是"plugin 读较新者"的全部含义：你用 CC 时桥接层写得更勤，菜单栏跟着 CC 实时刷新；你不用 CC 时数据层每分钟级别更新。
...
### 4.3 菜单栏标题状态机（`title_line`，`plugin/claude-gauge.15s.sh:140-164`）
...
- **5 小时窗口无前缀**（即"当前"），**周窗口加 `W` 前缀**——`s5()` 与 `s7()` 函数，`:147-148`。
- **额外用量提示**：若 `extra_usage.is_enabled` 且 `used_credits > 0`，且当前是够用状态、加上后宽度仍 ≤ `MAXW`，标题追加 `+$`（`:156`）。
...
- `render()` 里 `age = time.time() - ts; stale = age > STALE_SEC`（`:173`）。
- 陈旧时：菜单栏标题变灰加 `~`（`title_line` 的 stale 分支，`:159`/`:162`），且下拉菜单插入警告块：`⚠️ 数据已 N 分钟未更新` + `闲置/限流；用一下 Claude Code 即刷新`（`:184-186`）。
- 这样陈旧数据一眼可辨，菜单栏不会停在过时的数字上。

### 4.6 下拉菜单结构（`render` + `section`，`:166-201`）

SwiftBar 的坑：一行若**既无点击动作又无颜色**，会被渲染成"禁用灰"。所以**每一行都显式上色**。

每个窗口由 `section(label, icon, u, cd_str, col)` 渲染三/四行（`:166-170`）：
...
下拉还包含：标题行 `Claude Code 用量`；按模型分项（若有 `seven_day_sonnet` / `seven_day_opus`，`:190-193`）；更新时间 `更新于 HH:MM（N分钟前/刚刚）`（`:195-196`）；一个可点击的 **`立即刷新（强制拉最新）`** 项——它 `shell` 调用 `~/.claude/claude-gauge-refresh.sh` 并传 `param0=force`，强制数据层立刻 poll（`:197`）；以及（仅当稳定卸载脚本 `~/.claude/claude-gauge-uninstall.sh` 存在时）一个 **`管理 ▸ 卸载 ClaudeGauge…`** 子菜单——`terminal=true` 在 Terminal 里可见地跑卸载脚本，收进子菜单不污染主下拉（`:198-201`，机制见 §8.6）。
...
渲染层也能读 token 自己拉一次（`read_token`，`plugin/claude-gauge.15s.sh:207-214`），但**仅在后台缓存彻底失效时**才触发——`best is None` 或 `best.ts` 比当前早超过 150 秒（`:219`）。它读 token 前会检查 `expiresAt` 是否还有 30 秒余量（`:221`），过期就不拉。注意渲染层**不做 token 续命**，续命是数据层的专责。
...
- **渲染层兜底阈值**：`plugin/claude-gauge.15s.sh:219` 的 `150` 秒（best 多久没更新才让渲染层自己拉）。

### 8.4 加模型分项

数据层已经在抓 `seven_day_sonnet` / `seven_day_opus`（`refresher/claude-gauge-refresh.sh:88` 的循环 key 列表），渲染层也已展示（`plugin/claude-gauge.15s.sh:175`、`:190-193`）。要加新模型分项：

1. 在数据层 `:88` 的 key 元组里加上新窗口名（前提是 API 返回该 key）。
2. 在渲染层 `render()` 里仿照 `son`/`opus` 取 `remain(...)` 并加进 `extras` 列表（`:175`、`:190-193`）。
...
- **`_armed()`（`:132-138`）**：要点亮彩虹须**同时**满足三条——(a) `attention.front != (attention.host or CLAUDE_BUNDLE)`（事件发生时你不在**会话载体**前台；`host` 缺失则回退比桌面端 bundle，`:136`），**且** (b) `attention.idle >= AWAY_SEC`（事件发生时你已离开键鼠 ≥ `AWAY_SEC`，`:137`；`AWAY_SEC=90.0` 是紧挨 `def _armed()` 上方的可调常量，`:131`），**且** (c) `attention.ts > ack.ts`（未读，`:138`）→ 返回 `True`。`idle` 门是新加的：旧规则只看「完成那一刻 Claude 窗口是否前台」，于是你人在、只是临时切到别的 App（如 Chrome）也会误亮；`idle >= 90s` 把「真的起身离开」和「在用电脑、只是切了下窗口（空闲≈0）」区分开。缺/旧 `idle` 字段读作 `0` → 不点亮（fail-quiet）。
- **回到会话载体自动熄灭**：`render()`（自动熄灭判定在 `:178-179`）每次重画时若检测到当前前台 == `attention.host or CLAUDE_BUNDLE`（`_front_bundle()`，`:110-120`，同样只用 `lsappinfo`）且有未读，就写一份新的 `ack.json` → 下次重画 `_armed` 转 `False`、彩虹熄灭（≤15s 内自动恢复常态）。即终端会话回终端、桌面会话回桌面端都能自动熄灭。此自动熄灭逻辑未受 idle 门影响。
- **armed 渲染**（`title_line` 的 armed 分支，`:157-161`）：菜单栏标题用 `image={ICON_RAINBOW} {ICON_SZ}` 全彩位图 **+ 左键动作** `bash=/usr/bin/python3 param0=<alert.py> param1=open terminal=false`；**数字仍保留额度三色**（橙/红恒显，够用态不写 `color=` 走菜单栏自适应），只把图标染彩虹——两个信号共存，不遮蔽额度告急。普通（未 armed）态走原版图标常量（够用态 `templateImage={ICON_OK}`、橙/红/灰 `image={ICON_WARN/CRIT/STALE}`，`:162-164`），与 `ICON_RAINBOW` 同源同形（均由 `build-menubar-icons.sh` 从 `docs/logo.svg` 渲），故彩虹态与普通态**形状 100% 一致**。
...
- **管理子菜单**：渲染层 `render()`（`:198-201`）在「立即刷新」之后，**仅当** `~/.claude/claude-gauge-uninstall.sh` 存在时，打印一个 `管理 ▸ 卸载 ClaudeGauge…` 子菜单（`shell=/bin/bash param0=<那份稳定副本> terminal=true`）。`terminal=true` 让卸载在 Terminal 里可见地跑（用户能看到输出与提示）；用子菜单收纳、不污染主下拉。
