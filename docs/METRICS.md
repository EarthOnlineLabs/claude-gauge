...
> 安装方式只有 `git clone`（无 release 资产、无 cask、无 curl|bash），所以**没有 release 下载数**可看；采纳量靠下面这些信号。全部用 `gh`（已以 `EarthOnlineLabs` 身份认证、有 push 权限，traffic 端点需要 push 权限）。

| 信号 | 命令 | 看什么 | 坑 |
|---|---|---|---|
| Star / Fork / Watcher | `gh api repos/EarthOnlineLabs/claude-gauge --jq '{stars:.stargazers_count,forks:.forks_count,watchers:.subscribers_count}'` | 社会证明的头号指标，推广落地后重点盯 star 增长 | 累计值无时间序列（要 star 时间线用 stargazers + `Accept: application/vnd.github.star+json`） |
| 仓库流量·浏览 | `gh api repos/EarthOnlineLabs/claude-gauge/traffic/views` | 仓库页 PV/UV，确认「落地页→仓库」漏斗 | **14 天滚动窗口，GitHub 不留存**；含自己访问 |
| 仓库流量·clone | `gh api repos/EarthOnlineLabs/claude-gauge/traffic/clones` | 最接近「有人拉代码去装」的代理指标 | 同 14 天窗口；**clone 数被 CI/镜像/依赖 bot 严重灌水，是上界不是真人数**；clone ≠ install |
| 来源 referrer | `gh api repos/EarthOnlineLabs/claude-gauge/traffic/popular/referrers` | 谁把流量带到仓库（含落地页、各平台） | 仅 Top 10；14 天 |
| 热门路径 | `gh api repos/EarthOnlineLabs/claude-gauge/traffic/popular/paths` | 哪个 README（en/zh）更受关注 | 仅 Top 10；14 天 |
| Release 下载数 | `gh api repos/EarthOnlineLabs/claude-gauge/releases` | 每个 release 资产的 `download_count` | **当前为空**：0 release、0 tag。除非开始打 tagged release 并附资产，否则永远空 |
...
- **直链仓库**：`https://github.com/EarthOnlineLabs/claude-gauge?ref=twitter` / `?ref=xiaohongshu`（让来源出现在 GitHub traffic referrers 里）
