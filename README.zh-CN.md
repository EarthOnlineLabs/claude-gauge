...
<p align="center">
  <img src="docs/screenshots/menubar.png" alt="ClaudeGauge 菜单栏显示" width="640">
</p>
...
- **额度没数。** Claude Code 的用量只能靠 `/usage` 现查，查一次还得切出当前对话。
- **撞墙没预警。** 5 小时窗口和一周窗口各跑各的，往往是限流弹出来了才发现。
- **旧数据骗人。** 大多数"用量小工具"在闲置或限流时会继续显示几小时前的数字，让你以为还很宽裕。

ClaudeGauge 的设计原则只有一条：**要么显示真实的当前用量，要么明确告诉你"数据陈旧、别信"——绝不让旧数据冒充新数据。**
