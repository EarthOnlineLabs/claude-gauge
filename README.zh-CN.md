...
把长任务丢给它，自己去忙别的。当某个 Claude Code 会话**完成**（它的 `Stop` 事件）时，菜单栏的表盘会亮起彩虹，把你叫回来。点一下就把 Claude 桌面 App 拉回前台，彩虹随即熄灭。百分比照常保持原来的颜色，只有图标变彩虹——额度告急不会被盖住。
...
它**只对 Claude Code 自己的「完成」(`Stop`) 事件作反应**——绝不读你的对话或代码，不弹窗，也不收集任何数据。安装会把两条 hook 安全地合并进你的 `~/.claude/settings.json`（先备份、幂等，且绝不动你已有的 hooks）；卸载只删它自己加的条目。
